...
def load_data(activity_data_dir, has_uncleaned = True):
    ...
    for name in pkl_names:
        if not os.path.exists(os.path.join(uncleaned_data_dir, name)):
            ...
            clean_dfs = clean_all_data(uncleaned_data_dir, create_dir=True)
            break
        # if not, clean the data and save it
    if clean_dfs == {}:
        print("Pickled data found in activity data directory. Loading data...")
        clean_dfs = load_cleaned_data(uncleaned_data_dir)
    return clean_dfs

def load_data2(activity_data_dir, live_json = True, live_csv = False, pp_csv = True):

    clean_dfs = clean_all_data2(activity_data_dir, create_dir=True)
    return clean_dfs
...
def run_cleaning_process(b3_df, index = "breathTime", demo = False):
    ...
    :param b3_df: breath-by-breath dataframe with
    :param index: the name of the column that will be made into the index
    :param demo: if true, return all intermediate dataframes
    :return:
    """
...
def get_raw_from_json(raw_json_path, reorder=True, process=True):
    ...
    if len(rtBreathing) > 0:
        ...
        if reorder:
            ...
            order = [name for name in AWS_NAME_MAP_2_LIVE.values() if name in rt_df.columns]
...
def clean_all_data(uncleaned_data_dir, create_dir = True, pickle_data = True):
    ...
    if create_dir:
        ...
        cleaned_folder_path = uncleaned_data_dir
...
def clean_all_data2(uncleaned_data_dir, create_dir = False, pickle_data = False):
    def pickle_all(cleaned_data_dir, df_dict):
        for df_name, df in df_dict.items():
            file_path = os.path.join(cleaned_data_dir, df_name + '.pkl')
            pkl.dump(df, open(file_path, 'wb'))
    ...
    for file_name in os.listdir(uncleaned_data_dir):
        if file_name.endswith('.csv'):
            csv_file_paths.append(os.path.join(uncleaned_data_dir, file_name))
    # check if there are more than 2 csv files (should have one for the processed data and possibly one for the live)
    if len(csv_file_paths) > 2:
        print("RECHECK ACTIVITY DIRECTORY - TOO MANY CSVs FOUND (SHOULD ONLY BE AT MOST 2 - ONE FOR PROCESSED AND ONE FOR LIVE)")
    elif len(csv_file_paths) < 1:
        print("RECHECK ACTIVITY DIRECTORY - NO CSVs FOUND")

    # check if one of the csv files has 'live' in it
    live_csv_path = None
    proc_csv_path = None
    for csv_path in csv_file_paths:
        if 'live' in csv_path:
            ...
            proc_csv_path = csv_path
    if live_csv_path is None:
        print("NO LIVE CSV DETECTED")
    if proc_csv_path is None:
        print("NO PROCESSED CSV DETECTED")
    ...
    dd = {} # dataframe dictionary
    if raw_json_path is not None:
    # Get the raw 125Hz data (fast_df), raw 25Hz data (slow_df), and real-time breath-by-breath data (rt_b3_df) from the "{}_Raw.csv"
        dd["raw_fast_df"], dd["raw_slow_df"], dd["live_b3_df"] = get_raw_from_json(raw_json_path)

    if proc_csv_path is not None:
        # Get the processed time-dependent data and the breath-by-breath dependent data from the "Processed data-{}.csv"
        dd["aws_time_df"], aws_b3_df = get_aws_data_from_csv(proc_csv_path)
        ## Get raw chest for each breath detection time
        # AWS
    ...
    if live_csv_path is not None:
        dd["c_df"] = pd.read_csv(live_csv_path)
        dd["c_df"].rename(columns = {"PeakTS": "breathTime", "AvgBR": "RRAvg", "InstBR": "instBR"}, inplace = True)
